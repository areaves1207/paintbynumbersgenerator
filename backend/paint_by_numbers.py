...
def paint_by_numbers_gen(img, num_colors, force_scale = True):
    ...
    print("K-means clustering finished in: %f seconds", (start_time - time.time()) )

    print("edge detector")
    start_time = time.time()
    edges = edge_detector.detect_edges_canny(clustered_img.copy())
    print("Canny edge detector finished in: %f seconds", (start_time - time.time()) )
    start_time = time.time()
    edges_tight = edge_detector.detect_edges_tight(clustered_img.copy())
    print("Tight edge detector finished in: %f seconds", (start_time - time.time()) )
    ...
    print("Combined edges and photos in: %f seconds", (start_time - time.time()) )
    ...
    print("Numbers configured in: %f seconds", (start_time - time.time()) )
    ...
    print("Padding and pallete completed in: %f seconds", (start_time - time.time()) )
    ...
    print("Image finalized in: %f seconds", (start_time - time.time()) )
