...
def generate_palette(img_height, color_palette):
    ...
    palette = np.full(shape=(img_height, box_size * 7, 3), fill_value=[255,255,255], dtype=np.uint8)
    ...
    for c, color_idx in enumerate(range(0, num_colors), start=1): #loop through each color
        ...
        cv.putText(img=palette, text=str(color_idx+1), org=(int(box_size + 2), y + int(box_size//1.25)), fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=0.04*box_size, color=(0,0,0), thickness=1, lineType=cv.LINE_AA)
        #Write rgb vals
        cv.putText(img=palette, text=str(color), org=(0, y + int(box_size*1.5)), fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=0.015*box_size, color=(0,0,0), thickness=0, lineType=cv.LINE_AA)
